...
public class HW201214_lottoField extends Frame implements ActionListener{
	Button btn;
	TextField tf1, tf2, tf3, tf4, tf5, tf6;		//텍스트 필드 각각 선언, 왜 배열로는 동작 안하는지 아직 모름
	int []temp = new int[6];					//이후 랜덤 배열 6개를 위한 temp
	...
	public HW201214_lottoField() {
		...
		tf1 = new TextField("",4);					//위치 예쁘게 배열함
		...
		btn.addActionListener(this);	//버튼 동작 리스너
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == btn) {				// 버튼 구분
			temp = getRandom();
			tf1.setText(Integer.toString(temp[0]));		//int를 String으로 바꾸고 setText로 할당해주는것이다.
